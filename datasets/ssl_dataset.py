...
class ImagenetDataset(torchvision.datasets.ImageFolder):
    ...
    def __getitem__(self, index):
        ...
        if not self.ulb:
            return index, sample_transformed, target
        else:
            if self.algo == 'fixmatch':
                return index, sample_transformed, self.strong_transform(sample)
            elif self.algo == 'flexmatch':
                return index, sample_transformed, self.strong_transform(sample)
            elif self.algo == 'pimodel':
                return index, sample_transformed, self.transform(sample)
            elif self.algo == 'pseudolabel':
                return index, sample_transformed
            elif self.algo == 'vat':
                return index, sample_transformed
            elif self.algo == 'meanteacher':
                return index, sample_transformed, self.transform(sample)
            elif self.algo == 'uda':
                return index, sample_transformed, self.strong_transform(sample)
            elif self.algo == 'mixmatch':
                return index, sample_transformed, self.transform(sample)
            elif self.algo == 'remixmatch':
                rotate_v_list = [0, 90, 180, 270]
                rotate_v1 = np.random.choice(rotate_v_list, 1).item()
                img_s1 = self.strong_transform(sample)
                img_s1_rot = torchvision.transforms.functional.rotate(img_s1, rotate_v1)
                img_s2 = self.strong_transform(sample)
                return index, sample_transformed, img_s1, img_s2, img_s1_rot, rotate_v_list.index(rotate_v1)
...
class ImageDatasetLoader:
    def __init__(self, root_path, num_labels=-1, num_class=1000, dataset='imagenet', algo='fixmatch'):
    ...
    def get_ulb_train_data(self):
        ...
        data = ImagenetDataset(root=os.path.join(self.root_path, "train"), transform=transform, ulb=True, algo=self.algo)
        return data
